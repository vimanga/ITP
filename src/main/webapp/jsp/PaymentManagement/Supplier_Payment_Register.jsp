...
<head>
	...
	<title>Supplier Payment Registry</title>
</head>
...
<div class="jumbotron"><h1>Supplier Payment Registry</h1></div>
...
<tr><th>Supplier Pay Id</th><th>Date</th><th>Order ID</th>
<th>Ammount</th><th>Supplier Name</th>
<th>Cheque Number</th><th>Amount</th><th></th></tr>  

   <c:forEach var="supplierpayment" items="${list}"> 
   <tr>  
   <td>${supplierpayment.supplierpayID}</td>  
   <td>${supplierpayment.date}</td> 
   <td>${supplierpayment.supOrderID}</td>  
   <td>${supplierpayment.amount}</td>
   <td>${supplierpayment.supName}</td> 
   <td>${supplierpayment.chequeNumber}</td>  
   <td>${supplierpayment.bank}</td> 
  
 	<td><a href="deletesuppayentry/${supplierpayment.supplierpayID}" onclick="return confirm('Are you Sure?');">Delete</a></td> 
...
<button class="btn btn-primary" onclick="window.open('supplierPayreport')">Report</button>
