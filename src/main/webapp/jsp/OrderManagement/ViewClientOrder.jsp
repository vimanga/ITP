...
<body>
	...
	<div class="container-fluid" style="height: 2000px">
		<div class="row" >
			...
			<h2 align="center" style="margin-top: 2%">Client Orders</h2>
				<table id="t01" class="table table-striped table-bordered">
					...
					<th>Confirm</th><th>Edit</th><th>Delete</th>
					...
					<tr>
						...
						<td> ${employee.exdate}</td>
						<td> ${employee.paytyp}</td>
						<td> ${employee.deltyp}</td>
						<td> ${employee.description}</td>
						<td>
							<c:choose>
								<c:when test="${employee.status}"><a href="#">confirmed</a></c:when>
								<c:otherwise><a href="/confirmClientOrder?id=${employee.oid}&item=${employee.item}&quantity=${employee.quantity}">confirm</a></c:otherwise>
												</c:choose>
						
							</td>
						<td><a href="/EditClientOrder?id=${employee.oid}" >Edit</a></td>
						<td><a href="/delete?id=${employee.oid}" onclick="return confirm('Are you sure you want to delete order');">Delete</a></td>
					</tr>
				</c:forEach>
