			...
			<div class="col-md-12">
				<form:form  method ="POST" modelAttribute="inventory" id="validateForm1" class="form-horizontal form-responsive" action = "iregister">
  				...
  				<div class="form-group row" style="margin-left: 30%;margin-top: 2%">
    				<label class="control-label col-sm-2" for="deviceID">Device Name</label>
    					<div class="col-sm-4">
    						<select style ="width: 280px; height: 35px; padding: 8px;" id="deviceID" path="deviceID" name="deviceID">
    						<option value=""></option>
    						<%
    							try{
    								String sql = "select deviceID, name from device ";
    								...
    								while (rs.next()){
    									...
    									<option value = "<%= rs.getInt("deviceID") %>"> <%= rs.getString("name") %></option>
    									<%
    								}
    							}
	 			...
	 			<div class="form-group" style="margin-left: 32%;margin-top: 2%"> 
 	 			...
 	 			<div class="row" >
  					<div class="col md-5 offset-4 mt-5" align="center">
  						<button type="button" class="btn btn-primary" onclick="window.open('inreport')">Generate Inventory Report</button>
  					</div>
  				</div>
				</form:form>
			</div>
